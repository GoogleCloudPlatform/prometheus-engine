...
func TestPrometheusTSDBCompatibilityPinned(t *testing.T) {
	e, err := e2e.New()
	t.Cleanup(e.Close)
	testutil.Ok(t, err)
	...
	snapshotDir := obtainSnapshot(t, e, "quay.io/bwplotka/prometheus:v3-index") //fmt.Sprintf("%v:%v", promImage, end.Original()))
	ts := time.Now()
	tsFn := func() time.Time { return ts }

	pEnd := newReadOnlyPrometheusFromSnapshot(t, e, "prom-"+end.Original(), fmt.Sprintf("%v:%v", promImage, end.Original()), snapshotDir)
	pStart := newReadOnlyPrometheusFromSnapshot(t, e, "prom-"+start.Original(), fmt.Sprintf("%v:%v", promImage, start.Original()), snapshotDir)
	testutil.Ok(t, e2e.StartAndWaitReady(pEnd, pStart))

	testutil.Ok(t, expectAtLeastSamples(t, pEnd, tsFn, expectedAtLeastSamples))
	testutil.Ok(t, expectAtLeastSamples(t, pStart, tsFn, expectedAtLeastSamples))

	testutil.Ok(t, e2einteractive.OpenInBrowser("http://"+pEnd.Endpoint("http")))
	testutil.Ok(t, e2einteractive.OpenInBrowser("http://"+pStart.Endpoint("http")))
	testutil.Ok(t, e2einteractive.RunUntilEndpointHit())
}

func TestPrometheusTSDBCompatibility(t *testing.T) {
	e, err := e2e.New()
	t.Cleanup(e.Close)
	testutil.Ok(t, err)
