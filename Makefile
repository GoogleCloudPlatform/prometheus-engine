...
# If pushing, build and tag native arch image to GCR.
	...
	docker tag "gmp/$(BIN_GO_NAME):$(TAG_NAME)" "gmp/$(BIN_GO_NAME):latest"
	docker tag "gmp/$(BIN_GO_NAME):$(TAG_NAME)" "${IMAGE_REGISTRY}/$(BIN_GO_NAME):$(TAG_NAME)"
	docker push "${IMAGE_REGISTRY}/$(BIN_GO_NAME):$(TAG_NAME)"
...
# Just build it locally.
	...
	docker build --tag "gmp/$(BIN_GO_NAME):$(TAG_NAME)" -f "./$(BIN_GO_DIR)/$(BIN_GO_NAME)/Dockerfile" .
	docker tag "gmp/$(BIN_GO_NAME):$(TAG_NAME)" "gmp/$(BIN_GO_NAME):latest"
endif
