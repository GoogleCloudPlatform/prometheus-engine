...
func (p *Probe) GetEndpointStatus() []ScrapeEndpointStatus {
	return p.Status.EndpointStatuses
}

func (p *Probe) SetEndpointStatus(endpointStatus []ScrapeEndpointStatus) {
...
func targetScrapeConfig(id string, namespace string, ep ProbeTarget, relabelCfgs []*relabel.Config, limits *ScrapeLimits) ([]*promconfig.ScrapeConfig, error) {
	relabelCfgs = append(relabelCfgs, &relabel.Config{
		Action:      relabel.Replace,
		TargetLabel: "module",
		Replacement: ep.Module,
	})
			...
			ServiceDiscoveryConfigs: discovery.Configs{
				&discoverykube.SDConfig{
					...
					Selectors: []discoverykube.SelectorConfig{
						{
							Role:  discoverykube.RolePod,
							Field: fmt.Sprintf("spec.nodeName=$(%s),metadata.namespace=%s", EnvVarNodeName, namespace),
							// Pod name is random (generated by Deployment's ReplicaSet)
							Label: fmt.Sprintf("app.kubernetes.io/name=%s", BLACKBOX_EXPORTER_POD_NAME),
						},
					},
				},
			},
