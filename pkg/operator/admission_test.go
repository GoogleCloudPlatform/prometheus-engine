...
func TestAdmitServiceMonitoring(t *testing.T) {
	cases := []struct {
		doc      string
		resGroup string
		resVer   string
	...
	for _, c := range cases {
		t.Run(c.doc, func(t *testing.T) {
			// Prepare PodMonitoring resource.
			res := &v1alpha1.ServiceMonitoring{}
			res.Spec.TargetLabels.FromPod = c.mappings
			...
			// Send admission request to service monitoring validation.
			if resp, err := admitServiceMonitoring(&v1.AdmissionReview{
				Request: req,
			}); err != nil && !c.expErr {
				t.Errorf("returned unexpected error: %s", resp.Result.Message)
			} else if err == nil && c.expErr {
	...
	t.Run("bad request bytes", func(t *testing.T) {
		...
		if _, err := admitServiceMonitoring(&v1.AdmissionReview{
			Request: req,
		}); err == nil {
			t.Errorf("should have returned an error")
		}
