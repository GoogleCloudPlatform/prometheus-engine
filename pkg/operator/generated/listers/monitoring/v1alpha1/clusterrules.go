...
type ClusterRulesLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ClusterRules, err error)
	// Get retrieves the ClusterRules from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ClusterRules, error)
...
// List lists all ClusterRules in the indexer.
func (s *clusterRulesLister) List(selector labels.Selector) (ret []*v1alpha1.ClusterRules, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ClusterRules))
	})
	return ret, err
}

// Get retrieves the ClusterRules from the index for a given name.
func (s *clusterRulesLister) Get(name string) (*v1alpha1.ClusterRules, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("clusterrules"), name)
	}
	return obj.(*v1alpha1.ClusterRules), nil
}
