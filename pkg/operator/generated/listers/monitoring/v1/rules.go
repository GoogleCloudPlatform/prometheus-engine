...
// List lists all Rules in the indexer.
func (s *rulesLister) List(selector labels.Selector) (ret []*v1.Rules, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Rules))
	})
	return ret, err
}
...
// List lists all Rules in the indexer for a given namespace.
func (s rulesNamespaceLister) List(selector labels.Selector) (ret []*v1.Rules, err error) {
...
// Get retrieves the Rules from the indexer for a given namespace and name.
func (s rulesNamespaceLister) Get(name string) (*v1.Rules, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("rules"), name)
	}
	return obj.(*v1.Rules), nil
}
