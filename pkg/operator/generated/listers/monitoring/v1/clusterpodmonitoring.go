...
// List lists all ClusterPodMonitorings in the indexer.
func (s *clusterPodMonitoringLister) List(selector labels.Selector) (ret []*v1.ClusterPodMonitoring, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.ClusterPodMonitoring))
	})
	return ret, err
}

// Get retrieves the ClusterPodMonitoring from the index for a given name.
func (s *clusterPodMonitoringLister) Get(name string) (*v1.ClusterPodMonitoring, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("clusterpodmonitoring"), name)
	}
	return obj.(*v1.ClusterPodMonitoring), nil
}
