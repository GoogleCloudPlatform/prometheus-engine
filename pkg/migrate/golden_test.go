...
func buildEndpointRelabelRules(gmpRelabelConfigs, gmpMetricRelabelConfigs, expectedRelabelConfigs []*relabel.Config, endpointPort string, svcLabels map[string]string) []*relabel.Config {
	...
	out = append(out,
		...
		newPORelabelRule(relabel.Replace, []string{"__meta_kubernetes_pod_name"}, "pod", "(.*)", "$1"),
		newPORelabelRule(relabel.Replace, []string{"__meta_kubernetes_pod_container_name"}, "container", "(.*)", "$1"),
		newPORelabelRule(relabel.Drop, []string{"__meta_kubernetes_pod_phase"}, "", "(Failed|Succeeded)", "$1"),
	)

	// 2. Extract Pod target labels if generated by GMP's fromPod compiler.
	for _, r := range gmpRelabelConfigs {
		if r == nil {
			continue
		}
	...
	// 3. Extract Service target labels migrated by gmp-migrate into metric relabel configs.
...
func extractCustomRelabelRules(gmpRelabelConfigs []*relabel.Config, role promk8s.Role) []*relabel.Config {
	...
	for _, r := range gmpRelabelConfigs {
		...
		if len(r.SourceLabels) == 1 {
			...
			if role == promk8s.RoleEndpoint && r.Action == relabel.Keep && strings.HasPrefix(src, metaPodLabelPrefix) {
				continue
			}
		}
		var src []string
