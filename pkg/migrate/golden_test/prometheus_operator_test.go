...
type podMonitorTestCase struct {
	name        string
...
func validateResourceAgainstSchema(t *testing.T, validators *gmpSchemaValidators, u *unstructured.Unstructured) {
	...
	switch u.GetKind() {
	case migrate.KindPodMonitoring:
		validator = validators.podMonitoring
	case migrate.KindClusterPodMonitoring:
...
func TestPrometheusOperatorOpenAPIGolden(t *testing.T) {
	...
	goldenDir := filepath.Join("testdata", "golden", "podmonitor")

	var allCases []podMonitorTestCase
	allCases = append(allCases, upstreamPodMonitorTestCases...)
	allCases = append(allCases, customPodMonitorTestCases...)
	...
	for _, tc := range allCases {
		...
		t.Run(testName, func(t *testing.T) {
			...
			tc.input.TypeMeta = metav1.TypeMeta{APIVersion: "monitoring.coreos.com/v1", Kind: migrate.KindPodMonitor}
			...
			for _, obj := range tc.extraInputs {
				objBytes, err := sigyaml.Marshal(obj.Object)
				if err != nil {
					t.Fatalf("failed to marshal extra input: %v", err)
				}
				tmpPath := filepath.Join(testDir, fmt.Sprintf("extra_%s_%s.yaml", obj.GetKind(), obj.GetName()))
				if err := os.WriteFile(tmpPath, objBytes, 0644); err != nil {
					t.Fatalf("failed to write extra input file: %v", err)
				}
				defer os.Remove(tmpPath)
				extraPaths = append(extraPaths, tmpPath)
			}
			...
			allInputPaths := append([]string{inputPath}, extraPaths...)
			report, err := migrator.Run(allInputPaths...)
			if err != nil {
