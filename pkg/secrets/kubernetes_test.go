...
func TestProvider(t *testing.T) {
	...
	t.Run("valid secret watch cancelled", func(t *testing.T) {
		...
		providerSecret, err := provider.Add(toSecretConfig(typeBinary.secret, key))
		require.NoError(t, err)
		requireFetchEquals(t.Context(), t, providerSecret, typeBinary.entries[0].value)
		...
		updateKey(secret, key, "Goodbye")
		_, err = c.CoreV1().Secrets(secret.Namespace).Update(t.Context(), secret, metav1.UpdateOptions{})
		require.NoError(t, err)

		// Now that we've updated the secret, let the watcher reload.
		waitMutex.Unlock()
		requireFetchEquals(t.Context(), t, providerSecret, "Goodbye")
		...
		requireFetchEquals(t.Context(), t, providerSecret, "Hello")
