...
class GrammarDeclaration extends Node {
    constructor(start, rules, topRules, tokens, context, externalTokens, externalSpecializers, precedences, mainSkip, scopedSkip, dialects, externalProps, autoDelim) {
        super(start);
        this.rules = rules;
...
function parseGrammar(input) {
    ...
    return new GrammarDeclaration(start, rules, tops, tokens, context, external, specialized, prec, mainSkip, scopedSkip, dialects, props, autoDelim);
}
...
class Builder {
    ...
    prepareParser() {
        ...
        for (let i = 0; i < this.dialects.length; i++)
            dialects[this.dialects[i]] = data.storeArray((this.tokens.byDialect[i] || none).map(t => t.id).concat(65535 /* End */));
...
class FinishStateContext {
    ...
    finish(state, isSkip, forcedReduce) {
        ...
        this.stateArray[base + 1 /* Actions */] = this.storeActions(defaultReduce ? none : state.actions, skipReduce, shared);
