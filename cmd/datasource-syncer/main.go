...
func main() {
	...
	if *grafanaAPIToken != "" && *grafanaAPITokenFilepath != "" {
		logger.Error("at most one of --grafana-api-token and --grafana-api-token-filepath must be set")
		os.Exit(1)
	}
	...
	if *grafanaAPITokenFilepath != "" {
		...
		if err != nil {
			logger.Error("could not read grafana-api-token-filepath", "err", err)
			os.Exit(1)
		}
	...
	if *grafanaAPIToken == "" {
		...
		if envToken == "" {
			logger.Error("at most one of --grafana-api-token, --grafana-api-token-filepath, or the environment variable GRAFANA_SERVICE_ACCOUNT_TOKEN must be set")
			os.Exit(1)
		}
