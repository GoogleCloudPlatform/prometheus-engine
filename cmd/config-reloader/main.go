...
func main() {
	var (
		...
		configMapSelector  = flag.String("config-dir-from-configmap-selector", "", "label selector to discover ConfigMaps via the Kubernetes API (e.g. monitoring.googleapis.com/rules-shard=true). When set, entries of the matching ConfigMaps are materialized into --config-dir, which must be writable by this process.")
