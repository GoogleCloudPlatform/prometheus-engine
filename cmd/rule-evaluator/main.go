...
func main() {
	...
	reloadCh := make(chan chan error)
	{
		...
		http.HandleFunc("/api/v1/rules/test/test", http.NotFound) // Magical endpoint used by Grafana to check whether a datasource has the ruler-api enabled. Should contain the words "page not found" in the response.
