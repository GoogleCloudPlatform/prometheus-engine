...
combine() {
  ...
  # No reordering to match older behavior of `combine` method.
  kubectl kustomize --reorder="none" "${source_dir}" >> $dest_yaml
}
...
update_crdgen() {
  ...
  CRD_YAMLS=$(find ${CRD_DIR} -iname '*.yaml' | sort | grep -v kustomization.yaml)
  ...
  combine $CRD_DIR ${SCRIPT_ROOT}/manifests/setup.yaml
}
...
update_manifests() {
  ...
  combine $CRD_DIR ${SCRIPT_ROOT}/manifests/setup.yaml
  combine $OP_DIR ${SCRIPT_ROOT}/manifests/operator.yaml
  combine $RE_DIR ${SCRIPT_ROOT}/manifests/rule-evaluator.yaml
}
