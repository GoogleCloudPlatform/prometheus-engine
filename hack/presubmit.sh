...
update_crdgen() {
  ...
  combine $CRD_DIR '^.*/.*.yaml$' ${REPO_ROOT}/manifests/setup.yaml
}
...
update_manifests() {
  ...
  OP_DIR=${REPO_ROOT}/cmd/operator/deploy/operator
  sort_manifests $OP_DIR
  RE_DIR=${REPO_ROOT}/cmd/operator/deploy/rule-evaluator
  sort_manifests $RE_DIR

  combine $CRD_DIR '^.*/.*.yaml$' ${REPO_ROOT}/manifests/setup.yaml
  combine $OP_DIR '^.*/[0-9][0-9]-\w.*.yaml$' ${REPO_ROOT}/manifests/operator.yaml
  combine $RE_DIR '^.*/[0-9][0-9]-\w.*.yaml$' ${REPO_ROOT}/manifests/rule-evaluator.yaml
}
