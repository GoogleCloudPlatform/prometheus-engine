...
update_codegen() {
	...
	CODEGEN_PKG=${CODEGEN_PKG:-$(
		cd "${REPO_ROOT}"
		go list -m -f '{{.Dir}}' k8s.io/code-generator 2>/dev/null || echo ../code-generator
	)}
...
combine() {
	...
	go tool -modfile="${REPO_ROOT}/tools/go.mod" yq '... comments=""' ${SOURCE_DIR}/*.yaml >>"$DEST_YAML"
}

update_crdgen() {
	echo ">>> regenerating CRD yamls"
	API_DIR=${REPO_ROOT}/pkg/operator/apis/...
	go tool -modfile="${REPO_ROOT}/tools/go.mod" controller-gen crd paths=${API_DIR} output:crd:dir=${CRD_DIR}
...
update_docgen() {
	...
	mkdir -p doc
	go tool -modfile="${REPO_ROOT}/tools/go.mod" gen-crd-api-reference-docs \
...
update_manifests() {
	...
	combine ${CRD_DIR} "${REPO_ROOT}/manifests/setup.yaml"
	go tool -modfile="${REPO_ROOT}/tools/go.mod" helm template "${REPO_ROOT}/charts/operator" \
		-f "${REPO_ROOT}/charts/values.global.yaml" \
		>"${REPO_ROOT}/manifests/operator.yaml"
	go tool -modfile="${REPO_ROOT}/tools/go.mod" helm template "${REPO_ROOT}/charts/rule-evaluator" \
		-f "${REPO_ROOT}/charts/values.global.yaml" \
...
reformat() {
	...
	popd
	go tool -modfile="${REPO_ROOT}/tools/go.mod" mdox fmt --soft-wraps "${REPO_ROOT}"/*.md "${REPO_ROOT}"/cmd/**/*.md "${REPO_ROOT}"/ops/gmpctl/*.md

	echo ">>> formatting bash scripts"
	# TODO: Fix and apply this to all .sh scripts we host.
	go tool -modfile="${REPO_ROOT}/tools/go.mod" shfmt -l -w "${REPO_ROOT}/ops/gmpctl/lib.sh" "${REPO_ROOT}/hack/presubmit.sh"
}
