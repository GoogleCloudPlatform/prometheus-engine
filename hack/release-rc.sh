...
Example use:
 * BRANCH=release/0.15 TAG=v0.15.4-rc.0 CHECKOUT_DIR=~/Repos/tmp-release bash hack/release-rc.sh
 * BRANCH=release-2.45.3-gmp TAG=v2.45.3-gmp.13-rc.0 CHECKOUT_DIR=~/Repos/tmp-release bash hack/release-rc.sh
 * BRANCH=release-0.27.0-gmp TAG=v0.27.0-gmp.4-rc.0 CHECKOUT_DIR=~/Repos/tmp-release bash hack/release-rc.sh
...
if release-lib::needs_push || ! git ls-remote --tags --exit-code origin "refs/tags/${TAG}" >/dev/null; then
