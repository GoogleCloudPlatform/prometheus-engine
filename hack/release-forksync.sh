...
create_fork_commit() {
	...
	release-lib::exclude_changes_from_last_commit "${EXCLUDE_RE}" "${DOCUMENTATION_INCLUDE_RE}" "google-patch[logic]: required upstream code modifications"
...
#
# NOTE: This is interconnected with release-lib:$EXCLUDE_RE variable that excludes some of those files
...
recreate_fork_base_files() {
	...
	ls "documentation/" | grep -v -E "${DOCUMENTATION_INCLUDE_RE}" | xargs rm -f --
...
BRANCH=${SOURCE_BRANCH} release-lib::idemp::clone "${DIR}"
