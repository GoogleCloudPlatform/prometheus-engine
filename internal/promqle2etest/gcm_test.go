...
// Package promqle2etest implements GMP acceptance tests using promqle2e framework.
//
// The main goal is to have a basic acceptance test on the non-trivial behaviours across multiple ingestion pipelines.
// NOTE: To use this test on CI, make sure to add GCM_SECRET envvar containing GCM API read and write access (and adjust go test timeout).
//
// See also https://github.com/GoogleCloudPlatform/opentelemetry-operations-collector/pull/271 for
// similar tests with various OpenTelemetry pipelines e.g. target --PromProto--> OpenTelemetry Collector --GCM API--> GCM.
...
// TestExportGCM_PrometheusCounter_WithCT tests a counter sample behaviour
// when the CT is provided in the scrape.
func TestExportGCM_PrometheusCounter_WithCT(t *testing.T) {
	...
	//nolint:promlinter // Test metric.
	counter := promauto.With(pt.Registerer()).NewCounterVec(prometheus.CounterOpts{
		Name:        "promqle2e_test_counter_ct_total",
		Help:        "Test counter used by promqle2e test framework for acceptance tests.",
		ConstLabels: map[string]string{"repo": "github.com/GoogleCloudPlatform/prometheus-engine"},
	}, []string{"foo"})
	var c prometheus.Counter

	// TODO(bwplotka): Sadly all backends we test don't actually use CTs yet, this will change soon, update it.
...
// TestExportGCM_PrometheusGauge tests a gauge sample behaviour.
func TestExportGCM_PrometheusGauge(t *testing.T) {
	...
	//nolint:promlinter // Test metric.
	counter := promauto.With(pt.Registerer()).NewGaugeVec(prometheus.GaugeOpts{
